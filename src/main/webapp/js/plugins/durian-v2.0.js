...
(function(W, D) {
	...
	$d.api.ajaxAPI = function() {
		...
		/**
		 * @method clear HTTP Header, Parameter
		 * @param none
		 * @returns none
		 */
		this.clear = function() {
		...
		/**
		 * @method $d.api.AjaxAPI.request Ajax
		 * @param method - POST, GET, PUT, DELETE, HEAD
		 * @param opt { url, method, async, datatype } Object define
		 * 				url
		 * 				method - GET, POST, PUT, DELETE (default는 GET)
		 * 				async - true : asynchronous , false : synchronous (default true)
		 * 				datatype - xml, json (default json)
		 * @return none
		 */
					...
					if (xhr.readyState === 4) {
						if (xhr.status === 200) {
							if (that.format === 'json') {
								...
								that.onSuccess(xhr.status, xhr.responseText, opt.options);
							}
						}
	...
	/**
	 * if didn't defined of log Object
	 */	
	if(!window.console)
	{   
		console = {};
	...
	if(strUrl.indexOf("/view/bassInfo/carMng.do") < 0 
			&& strUrl.indexOf("/view/mngr/statHistoryMng.do") < 0 
			&& strUrl.indexOf("/view/mngr/emgHistoryMng.do") < 0 
			&& strUrl.indexOf("/view/mngr/smsHistoryMng.do") < 0 
			&& strUrl.indexOf("/view/bassInfo/userMng.do") < 0){
