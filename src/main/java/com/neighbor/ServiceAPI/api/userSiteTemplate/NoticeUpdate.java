...
import com.neighbor.ServiceAPI.api.userSiteTemplate.NoticeAdd.MustParam;
 ...
 * <p/>
 */
public class NoticeUpdate extends AbsQuery<Map> {
	private static final Log logger = LogFactory.getLog(NoticeUpdate.class);
	...
	enum MustParam
	{
		TITLE, CTNT, CNTNTS_TYPE2_CD2, U_CNTNTS_NO, NOTICE_NO, CNTNTS_CLS_CD, CNTER_CD
	}
	
	enum OptionParam
	{
		attachFiles, NOTICE_CLS_CD2, CNTNTS_NO
	}	
	...
	public Map executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			String NOTICE_CLS_CD2 = mapParameter.get(MustParam.CNTNTS_CLS_CD.name()).toString();
			...
			mRequest.setAttribute("CNTNTS_NO", mapParameter.get(MustParam.U_CNTNTS_NO.name()));
			mRequest.setAttribute("NOTICE_CLS_CD2", NOTICE_CLS_CD2);
			...
			if(fileList.size() > 0) {
				...
				mapParameter.put("board_no", mapParameter.get(MustParam.NOTICE_NO.name()));
				mapParameter.put("cntnts_no", mapParameter.get(MustParam.U_CNTNTS_NO.name()));
				mapParameter.put("file_attch_id", CNTNTS_TYPE2_CD2);
				
				userSiteTemplateService.fileInfoDelete(mapParameter);
			...
			userSiteTemplateService.noticeUpdate(mapParameter);
