...
public class EasypayBatchkeyUpdate extends AbsQuery<Map> {

    private static final Log logger = LogFactory.getLog(EasypayBatchkeyUpdate.class);
    ...
    public Class getMustParameter() throws AbsException {
        // TODO Auto-generated method stub
        return EasypayBatchkeyUpdate.MustParam.class;
    }

    @Override
    public Class getOptionParameter() throws AbsException {
        // TODO Auto-generated method stub
        return EasypayBatchkeyUpdate.OptionParam.class;
    }
    ...
    enum OptionParam
    {
        EP_tr_cd, EP_trace_no, EP_order_no, EP_mall_id, EP_sessionkey, EP_encrypt_data, EP_user_id, EP_user_name
        , EP_product_nm, EP_product_amt, EP_pay_type
        , EP_tot_amt, EP_currency, EP_card_txtype, EP_req_type, EP_card_amt, EP_wcc, EP_card_no, EP_install_period, EP_noint
    }
    ...
    public Map executeAPI(HttpServletRequest req, HttpServletResponse res,
        ...
        try {
            ...
            /* ::: 쇼핑몰 지불 정보 설정                                                  */
            /* -------------------------------------------------------------------------- */
            final String GW_URL                 =  props.getProperty("Globals.Easypay.GW_URL");
            ...
            /* -------------------------------------------------------------------------- */
            final String CERT_FILE = props.getProperty("Globals.Easypay.CertFileDir");
            final String LOG_DIR = props.getProperty("Globals.Easypay.LogDir");
            ...
            /* -------------------------------------------------------------------------- */
            //[헤더]
            String tr_cd = getNullToSpace(mapParameter.get("EP_tr_cd").toString());           // [필수]요청구분
            String trace_no = getNullToSpace(mapParameter.get("EP_trace_no").toString());        // [필수]추적고유번호
            String order_no = getNullToSpace(mapParameter.get("EP_order_no").toString());        // [필수]주문번호
            String mall_id = getNullToSpace(mapParameter.get("EP_mall_id").toString());         // [필수]몰아이디
            //[공통]
            String sessionkey = getNullToSpace(mapParameter.get("EP_sessionkey").toString());      // [필수]암호화키
            String encrypt_data = getNullToSpace(mapParameter.get("EP_encrypt_data").toString());    // [필수]암호화 데이타

            /* -------------------------------------------------------------------------- */
            /* ::: 결제요청 - 가맹점 주문정보                                             */
            /* -------------------------------------------------------------------------- */
            // String memb_user_no = getNullToSpace(mapParameter.get("EP_memb_user_no").toString());     // [선택]가맹점 고객일련번호
            String user_id = getNullToSpace(mapParameter.get("EP_user_id").toString());          // [선택]고객 ID
            String user_nm = getNullToSpace(mapParameter.get("EP_user_name").toString());        // [필수]고객명
            //  String user_mail = getNullToSpace(mapParameter.get("EP_user_mail").toString());        // [필수]고객 E-mail
            // String user_phone1 = getNullToSpace(mapParameter.get("EP_user_phone1").toString());      // [필수]가맹점 고객 연락처1
            //  String user_phone2 = getNullToSpace(mapParameter.get("EP_user_phone2").toString());      // [선택]가맹점 고객 연락처2
            //  String user_addr = getNullToSpace(mapParameter.get("EP_user_addr").toString());        // [선택]가맹점 고객 주소
            //  String product_type = getNullToSpace(mapParameter.get("EP_product_type").toString());     // [필수]상품정보구분[0:실물,1:컨텐츠]
            String product_nm = getNullToSpace(mapParameter.get("EP_product_nm").toString());       // [필수]상품명
            String product_amt = getNullToSpace(mapParameter.get("EP_product_amt").toString());      // [필수]상품금액

            /* -------------------------------------------------------------------------- */
            /* ::: 배치결제 정보 설정                                                     */
            /* -------------------------------------------------------------------------- */
            String pay_type = getNullToSpace(mapParameter.get("EP_pay_type").toString());        // [필수]결제수단
            String tot_amt = getNullToSpace(mapParameter.get("EP_tot_amt").toString());         // [필수]총결제금액
            String currency = getNullToSpace(mapParameter.get("EP_currency").toString());        // [필수]통화코드
            String client_ip = "127.0.0.1";//request.getRemoteAddr();                                    // [필수]고객 IP
            String cli_ver = "W8";                                                       // [필수]클라이언트 버젼(W8로 고정)
            String complex_yn = "N";                                                        // [필수]복합결제유무(배치결제는 사용 안함)
            String escrow_yn = "N";                                                        // [필수]에스크로 여부(배치결제는 사용 안함)

            String card_txtype = getNullToSpace(mapParameter.get("EP_card_txtype").toString());     // [필수]처리구분
            String req_type = getNullToSpace(mapParameter.get("EP_req_type").toString());        // [필수]결제종류
            String card_amt = getNullToSpace(mapParameter.get("EP_card_amt").toString());        // [필수]결제금액
            String wcc = getNullToSpace(mapParameter.get("EP_wcc").toString());             // [필수]WCC
            String card_no = getNullToSpace(mapParameter.get("EP_card_no").toString());         // [필수]배치키
            String install_period = getNullToSpace(mapParameter.get("EP_install_period").toString());  // [필수]할부개월
            String noint = getNullToSpace(mapParameter.get("EP_noint").toString());           // [필수]무이자여부
            ...
            if (r_res_cd.equals("0000")) {
                ...
                queryParam.put("EP_BATCHKEY",r_card_no);
                queryParam.put("MBER_ID",user_id);
                _checkNullParameterValue(queryParam);

                bassInfoService.easypayBatchKeyUpdate(queryParam);
                //end
            }

            resultData.put("r_res_cd", r_res_cd);
            resultData.put("r_res_msg", r_res_msg);
            resultData.put("r_card_no", r_card_no);
