 ...
 * <p/>
 */
public class CarUseList extends AbsQuery<Map> {
	private static final Log logger = LogFactory.getLog(CarUseList.class);
	...
	public Map executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			String cnter_cd = mapParameter.get("CNTER_CD").toString();
			if(cnter_cd == null || "".equals(cnter_cd)) {
				cnter_cd = "NAT-1-001"; //이동약자(사용자) 메뉴구성
			}
			mapParameter.put("CNTER_CD", cnter_cd);
			
			String gubn_type = mapParameter.get("GUBN_TYPE").toString();
			mapParameter.put("gubn", gubn_type);
			
			Map contentsInfo = mainService.selectContentsInfo(mapParameter);
			
			resultData.put("contentsInfo", contentsInfo);
